...
void insert(tree*& tr, int x) {//в дерево tr число x
    tree* n = node(x);
    if (!tr) {//если изначального дерева нет то 
        tr = n;
    }
    else {
        tree* y = tr;
...
void inorder(tree* tr) {//симметричный обход (просто с лева на право)
    if (tr) {
        inorder(tr->left);
        cout << tr->inf << " ";
        inorder(tr->right);
...
int main() {
    ...
    int ans = ff1(tr, k);
    if (ans!=-1) {
        cout  << "Это число есть и его глубина: " << ans;
    }
    else {
